const i18n = {
    en: {
        nav_work: "Work",
        nav_studio: "Studio",
        nav_contact: "Contact",
        nav_services: "Services",
        hero_title: "Elevating the<br>Cinematic Vision.",
        hero_subtitle: "We’re a team of experienced creatives who love shaping ideas into finished stories. From color grading and sound design to visual effects, we bring the craft and care your project deserves. Let’s create something meaningful together.",
        hero_btn: "Discover our work",
        section_work: "Featured Projects",
        section_studio: "The Studio",
        studio_text_1: "Misterio Color Lab is a post-production boutique specializing in color grading and finishing. We believe in the power of color to tell stories, conveying the exact emotion in every frame.",
        studio_text_2: "Our approach combines cutting-edge technology with a unique artistic vision, working closely with directors and cinematographers to achieve their ultimate vision.",
        service_1: "Integrated Digital Lab",
        service_2: "Color Grading & HDR",
        service_3: "Mastering & Deliveries",
        service_4: "Sound Design & Mix",
        service_5: "VFX & Compositing",
        service_6: "Post-production Supervision",
        contact_title: "Contact Us",
        contact_subtitle: "Have a project in mind? Let's talk about how we can give it the final touch it deserves.",
        form_name: "Your Name",
        form_email: "Email Address",
        form_message: "Tell us about your project...",
        form_submit: "Send Message",
        footer_desc: "Crafting the visual soul of your story.",
        section_team: "Our Team",
        role_head_color: "Head of Color",
        role_senior_colorist: "Senior Colorist",
        role_post_coordinator: "Post Production Coordinator",
        role_producer: "Producer",
        role_junior_colorist: "Junior Colorist",
        location_madrid: "Madrid - SPAIN",
        location_mexico: "Mexico City - MEXICO",
        location_saopaulo: "Sao Paulo - BRAZIL"
    },
    es: {
        nav_work: "Trabajo",
        nav_studio: "Estudio",
        nav_contact: "Contacto",
        nav_services: "Servicios",
        hero_title: "Elevando la<br>Visión Cinematográfica.",
        hero_subtitle: "Somos un equipo de creativos con experiencia a los que les apasiona dar forma a las ideas para convertirlas en historias acabadas. Desde la corrección de color y diseño de sonido hasta los efectos visuales, aportamos el oficio y el cuidado que tu proyecto merece. Creemos juntos algo significativo.",
        hero_btn: "Descubre nuestro trabajo",
        section_work: "Proyectos Destacados",
        section_studio: "El Estudio",
        studio_text_1: "Misterio Color Lab es una boutique de post-producción especializada en etalonaje y finalización. Creemos en el poder del color para contar historias, transmitiendo la emoción exacta en cada fotograma.",
        studio_text_2: "Nuestro enfoque combina tecnología de vanguardia con una visión artística única, colaborando estrechamente con directores y directores de fotografía para alcanzar su máxima visión.",
        service_1: "Laboratorio Digital Integrado",
        service_2: "Gradación de Color y HDR",
        service_3: "Mastering y Entregas",
        service_4: "Diseño de Sonido y Mezcla",
        service_5: "VFX y Composición",
        service_6: "Supervisión de Post-producción",
        contact_title: "Contáctanos",
        contact_subtitle: "¿Tienes un proyecto en mente? Hablemos sobre cómo podemos darle el acabado final que merece.",
        form_name: "Tu Nombre",
        form_email: "Correo Electrónico",
        form_message: "Háblanos de tu proyecto...",
        form_submit: "Enviar Mensaje",
        footer_desc: "Creando el alma visual de tu historia.",
        section_team: "Nuestro Equipo",
        role_head_color: "Director de Color",
        role_senior_colorist: "Colorista Senior",
        role_post_coordinator: "Coordinador de Postproducción",
        role_producer: "Productor",
        role_junior_colorist: "Colorista Junior",
        location_madrid: "Madrid - ESPAÑA",
        location_mexico: "Ciudad de México - MÉXICO",
        location_saopaulo: "São Paulo - BRASIL"
    }
};
